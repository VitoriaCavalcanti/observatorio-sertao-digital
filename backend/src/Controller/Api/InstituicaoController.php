...
final class InstituicaoController extends AbstractController
{
    ...
    public function index(InstituicaoRepository $repo): JsonResponse { return $this->json(array_map($this->normalize(...), $repo->findBy(['statusCadastro' => Instituicao::CADASTRO_PUBLICADO], ['nome' => 'ASC']))); }
    #[Route('/{id}', name: 'api_instituicao_show', methods: ['GET'])]
    public function show(Instituicao $item): JsonResponse { if ($item->getStatusCadastro() !== Instituicao::CADASTRO_PUBLICADO) throw $this->createNotFoundException(); return $this->json($this->normalize($item)); }
