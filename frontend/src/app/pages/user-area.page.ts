    ...
    <main class="page">
      ...
      @else {
        ...
        <section class="workspace-section"><div class="section-heading"><div><p class="eyebrow">Instituições</p><h2>{{ instituicao.id ? 'Editar instituição' : 'Nova instituição' }}</h2></div></div>
          ...
          <div class="user-records">@for (item of dados().instituicoes; track item.id) { <article><div><span class="tag">{{ status(item.statusCadastro) }}</span><h3>{{ item.nome }}</h3></div><div class="record-actions"><button (click)="editarInstituicao(item)" [disabled]="item.statusCadastro === 'em_analise'">Editar</button><button (click)="enviar('instituicoes', item.id)" [disabled]="item.statusCadastro === 'em_analise' || item.statusCadastro === 'publicado'">Enviar para análise</button></div></article> }</div>
        </section>
        ...
        <section class="workspace-section"><div class="section-heading"><div><p class="eyebrow">Projetos</p><h2>{{ projeto.id ? 'Editar projeto' : 'Novo projeto' }}</h2></div></div>
          ...
          <div class="user-records">@for (item of dados().projetos; track item.id) { <article><div><span class="tag">{{ status(item.statusCadastro) }}</span><h3>{{ item.titulo }}</h3></div><div class="record-actions"><button (click)="editarProjeto(item)" [disabled]="item.statusCadastro === 'em_analise'">Editar</button><button (click)="enviar('projetos', item.id)" [disabled]="item.statusCadastro === 'em_analise' || item.statusCadastro === 'publicado'">Enviar para análise</button></div></article> }</div>
        </section>
...
export class UserAreaPage implements OnInit {
  ...
  status(s?: StatusCadastro): string { return ({ rascunho: 'Rascunho', em_analise: 'Em análise', publicado: 'Publicado', devolvido: 'Correção solicitada' } as Record<string, string>)[s || 'rascunho']; }
